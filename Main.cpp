...
#include "C:\Users\DIM\Documents\GitHub\stb\stb_image.h"
...
#include <C:\GLP\Project777\objloader.hpp>
#include <C:\GLP\Project777\controls.hpp>
...
int main(void)
{
	...
	unsigned char* data2 = stbi_load("planet.jpg", &widthP, &heightP, &nrChannelsP, 0);
	if (data2)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, widthP, heightP, 0, GL_RGB, GL_UNSIGNED_BYTE, data2);
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else
	{
	...
	*/
	...
	bool res2 = loadOBJ("planet.obj", vertices_planet, uvs_planet, normals_planet);
	...
	GLuint uvbufferP;
	glGenBuffers(1, &uvbufferP);
	glBindBuffer(GL_ARRAY_BUFFER, uvbufferP);
	glBufferData(GL_ARRAY_BUFFER, uvs_planet.size() * sizeof(glm::vec2), &uvs_planet[0], GL_STATIC_DRAW);

	GLuint normalbufferP;
	glGenBuffers(1, &normalbufferP);
	glBindBuffer(GL_ARRAY_BUFFER, normalbufferP);
	...
	do {
		...
		glm::mat4 MVP2 = ProjectionMatrix * ViewMatrix * ModelMatrix;
		...
		glUniform1i(glGetUniformLocation(programID, "myTextureSampler2"), 1);

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, texture);
		//glUniform1i(texture, 0);

		// PLANET NA TSEKARW GIATI DOULEUEI ANAPODA
		// SOS ME TO GL UNIFORM 1 I
		...
		glDrawArrays(GL_TRIANGLES, 0, vertices_planet.size()); // 12*3 indices starting at 0 -> 12 triangles
